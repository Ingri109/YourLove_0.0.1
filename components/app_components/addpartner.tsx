...
const AddPartner = ({ Userdata }: InfoUserProps) => {
    ...
    return (
        <>
            <div className="basis-0 flex flex-col justify-center items-center mt-[36px] lg:basis-1/2 lg:mt-[0px] ">
                ...
                <div className="flex items-center justify-center w-full relative mt-[28px] mb-[8px]">
                    <input type="text" value={idPartner} onChange={(e) => setIdPartner(e.target.value)} name="ID" placeholder="ID" className="bg-color3_3 bg-opacity-30 backdrop-blur-md outline-none border-[4px] border-color3_3 rounded-lg text-white p-1 text-[16px] font-medium placeholder:text-transparent" />
                    <label className="bg-color3_3 rounded-t-lg left-[152px] top-[-18px] absolute block text-white text-[12px] px-[10px] py-[0.5px]">ID Користувача</label>

                </div>
                {message !== '' ? <h2 className="text-[10px] uppercase font-semibold text-color3 mb-[2px]">{message}</h2> : null}
                <button onClick={AddYourLove} className="flex items-center bg-color2_2 rounded-[16px] py-[5px] px-[20px] text-center text-white shadow-[0_6px_10px_1px_rgba(0,0,0,0.3)] transition delay-100 duration-200 ease-in-out hover:bg-color2_1 hover:scale-105 hover:-translate-y-0.5 hover:shadow-[0_8px_15px_3px_rgba(0,0,0,0.5)]">Добавити партнера</button>

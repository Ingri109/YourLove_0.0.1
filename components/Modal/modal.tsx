...
const Modal = ({ Userdata, requestsInfo, chekModel, onClose }: ModalProps) => {
    ...
    const acceptRequest = async () => {
        if (Userdata.id) {
            ...
            console.log('2')
            const { error: error_delete1 } = await supabase.from('requests').delete().eq('id_user', requestsInfo[0].id_user)
            console.log('3')
            const { error: error_delete2 } = await supabase.from('requests').delete().eq('id_user', requestsInfo[0].id_partner)
            router.refresh()
        }
